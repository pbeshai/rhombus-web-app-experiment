...
function (App, Common, StateApp, Ricardo1) {
	...
	var Ricardo1Model = Common.Models.ViewModel.extend({
		isCorrect: function () {
	...
	Ricardo1States.Play = StateApp.ViewState.extend({
		name: "trial",
		view: "Ricardo1::play",
		modes: {
			waiting: 'waiting', userPaperQuestion: 'userPaperQuestion', initialDelay: 'initialDelay', revealChoices: 'revealChoices',
		...
		initialize: function () {
			...
			this.model = new Ricardo1Model({
				...
				errorRate: this.options.parentOptions.errorRate
			});
		...
		startPreTrial: function () {
			this.changeMode(this.modes.userPaperQuestion);
		},
		...
		handleInput: function (choice) {
			...
			if (mode === this.modes.userPaperQuestion) {
				var errorRate = this.model.get('errorRate');

				// generate a randomly different choice than the one pressed
				var errorChoice;
				do {
					errorChoice = randomChoice();
				}	while (errorChoice === choice);

				var userChoice = Math.random() > errorRate ? choice : errorChoice;
				console.log('setting user choice to ', userChoice, errorRate);

				// set the userChoice to be either what they just entered or an error choice
				this.model.set("userChoice", userChoice);

				// need to save change to model so it shows up in the view.
				this.model.save();

				this.participant.set("choice", null);
				this.startTrial();

			} else if (mode === this.modes.initialDelay || mode === this.modes.distractorDelay) {
				// ignore user input
				this.participant.set("choice", null);
		...
		viewOptions: function () {
			...
			console.log('view options are updating: ', viewOptions);
			return viewOptions;
		},
	...
	Ricardo1States.RepeatedPlay = StateApp.RepeatState.extend({
		name: "repeat",
		State: Ricardo1States.Play,
	...
	Ricardo1States.Conclusion = StateApp.ViewState.extend({
		name: "conclusion",
		view: "Ricardo1::conclusion",
	...
	Ricardo1States.BlockComplete = StateApp.ViewState.extend({
		name: "block-complete",
		view: "Ricardo1::block-complete",
