...
function (App, Common, Ricardo1) {

	var Ricardo1Views = {};

	Ricardo1Views.Play = {};

	Ricardo1Views.Play.Participant = Common.Views.ParticipantDisplay.extend({
		...
		beforeRender: function () {
			...
			this.setView(".message-text", new Ricardo1Views.Segment({ value: this.model.get("choice")}));
		}
	});
	...
	*/
	Ricardo1Views.Segment = App.BaseView.extend({
		template: "app/apps/Ricardo1/templates/segment",
		valueToSegments: {
			"A": {
				top: true,
				middle: true,
	...
	Ricardo1Views.Play.Layout = App.registerView("Ricardo1::play", App.BaseView.extend({
		template: "app/apps/Ricardo1/templates/grid",
		numRows: 5,
		numCols: 5,
		ParticipantView: Ricardo1Views.Play.Participant,
		...
		update: function (data) {
			...
			if (data.mode === "revealChoices") {
				...
				// read userChoice from data since it changes depending on what the user entered.
				userSegment.segmentsFromValue(data.userChoice).render();
	...
	Ricardo1Views.Conclusion = App.registerView("Ricardo1::conclusion", Backbone.View.extend({
		template: "app/apps/Ricardo1/templates/conclusion"
	}));

	Ricardo1Views.BlockComplete = App.registerView("Ricardo1::block-complete", Backbone.View.extend({
		template: "app/apps/Ricardo1/templates/block_complete",
